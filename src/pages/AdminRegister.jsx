...
import { register as registerUser, logout } from "../services/authService";
...
export default function AdminRegister() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (data.role !== "ADMIN") {
        logout();
        throw new Error("Admin registration must use an admin account.");
      }
      ...
      setSuccessMessage("Admin account created successfully. Redirecting to the dashboard...");
      setTimeout(() => {
        navigate("/");
      }, 1200);
    } catch (err) {
    ...
    <div className="grid min-h-screen grid-cols-1 font-sans md:grid-cols-2 bg-[#f8f7f4]">
      ...
      <div className="relative hidden flex-col justify-between overflow-hidden bg-[#0d3320] p-12 md:flex">
        ...
        <div className="relative z-10 flex flex-1 flex-col justify-center gap-8">
          <div>
            <h1 className="font-serif text-[clamp(36px,4vw,52px)] font-medium leading-[1.15] tracking-tight text-white">
              Create<br />a New<br /><em className="not-italic text-[#6ee7b7]">Admin</em> Account
            </h1>
            <p className="mt-5 max-w-[360px] text-sm leading-[1.75] text-[#dde7e2]">
              Register a trusted admin account to manage vehicles, bookings, guides, and site content.
            </p>
          </div>

          <div className="grid grid-cols-3 gap-3">
            {[
              { val: "Admin", label: "Trusted Access" },
              { val: "Secure", label: "Role-based Sign-up" },
              { val: "Instant", label: "Dashboard Access" },
            ].map((stat) => (
        ...
        <div className="relative z-10 text-xs tracking-wider text-white/25">
          ExploreCeylon © {new Date().getFullYear()} · Admin Registration
        </div>
      </div>
      ...
      <div className="flex flex-col items-center justify-center p-12 bg-[#f8f7f4]">
        <div className="w-full max-w-[420px]">
          <div className="mb-9">
            <div className="mb-2.5 text-[11px] font-semibold uppercase tracking-[0.2em] text-[#059669]">Admin Registration</div>
            <h2 className="font-serif text-4xl font-semibold leading-[1.1] tracking-tight text-[#0d1a12]">Create Admin Account</h2>
            <p className="mt-2 text-sm text-[#6b7c71]">Only trusted staff may register an admin profile.</p>
          </div>

          <form onSubmit={handleSubmit}>
            {/* Full Name */}
            <div className="mb-5">
              <label className="mb-2 block text-[13px] font-medium tracking-wide text-[#2d4a38]" htmlFor="fullName">Full Name</label>
              <div className="relative">
                <span className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-[15px] text-[#9cb5a4]">👤</span>
                <input
                  id="fullName" type="text" required placeholder="Kasun Perera"
            ...
            <div className="mb-5">
              ...
              <div className="relative">
                <span className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-[15px] text-[#9cb5a4]">✉</span>
                <input
                  id="email" type="email" required placeholder="admin@example.com"
            ...
            <div className="mb-5">
              ...
              <div className="relative">
                <span className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-[15px] text-[#9cb5a4]">🔒</span>
                <input
                  id="password" type={showPassword ? "text" : "password"} required placeholder="Create a strong password"
            ...
            {/* Confirm Password */}
            <div className="mb-5">
              <label className="mb-2 block text-[13px] font-medium tracking-wide text-[#2d4a38]" htmlFor="confirmPassword">Confirm Password</label>
              <div className="relative">
                <span className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-[15px] text-[#9cb5a4]">🔒</span>
                <input
                  id="confirmPassword" type={showPassword ? "text" : "password"} required placeholder="Confirm your password"
            ...
            {successMessage && (
              <div className="mb-4.5 flex items-start gap-2.5 rounded-xl border border-green-200 bg-green-50 p-3 text-[13px] leading-relaxed text-green-700">
            ...
            >
              {loading ? (
                <><span className="mr-2 inline-block h-4 w-4 animate-spin rounded-full border-2 border-white/30 border-t-white align-middle" />Creating Admin...</>
              ) : ("Create Admin")}
            </button>
          </form>
          ...
          <div className="mt-4.5 flex items-center gap-2 rounded-xl border border-green-200 bg-green-50 px-3.5 py-3 text-xs text-green-800">
            <span>🔐</span><span>Admin registration creates a protected backend admin account.</span>
          </div>

          <p className="mt-4 text-center text-[12px] text-[#9cafa3]">
            Already registered?{" "}
            <Link to="/login" className="font-medium text-[#047857] hover:text-[#064e3b]">Sign in</Link>
          </p>
        </div>
