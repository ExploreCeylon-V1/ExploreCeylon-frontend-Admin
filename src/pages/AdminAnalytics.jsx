...
function ChartCard({ title, children, empty, height = "h-64" }) {
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <h2 className="text-base font-semibold text-slate-950 mb-4">{title}</h2>
      {empty ? (
        <div className={`${height} flex items-center justify-center text-sm text-slate-400`}>No data yet.</div>
      ) : (
    ...
    <div className="min-h-screen px-4 py-5 xl:px-10 xl:py-8">
      <main className="space-y-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Last 12 months, computed from live booking/user/review data.</p>
        </div>

        <section className="grid gap-4 lg:grid-cols-2">
          <ChartCard title="Monthly Registrations" empty={!hasAnyMonthlyData}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={registrations} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
          ...
          <ChartCard title="Monthly Trips Created" empty={!hasAnyMonthlyData}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={trips} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} />
                <XAxis dataKey="month" tick={{ fontSize: 12, fill: "#94a3b8" }} axisLine={{ stroke: "#e2e8f0" }} tickLine={false} />
                <YAxis tick={{ fontSize: 12, fill: "#94a3b8" }} axisLine={false} tickLine={false} allowDecimals={false} />
                <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid #e2e8f0", fontSize: 13 }} />
          ...
          <ChartCard title="Booking Trends (Vehicle vs Guide)" empty={!hasBookingData}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={bookings} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} />
                <XAxis dataKey="month" tick={{ fontSize: 12, fill: "#94a3b8" }} axisLine={{ stroke: "#e2e8f0" }} tickLine={false} />
                <YAxis tick={{ fontSize: 12, fill: "#94a3b8" }} axisLine={false} tickLine={false} allowDecimals={false} />
                <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid #e2e8f0", fontSize: 13 }} />
          ...
          <ChartCard title="Review Rating Distribution" empty={!hasReviewData}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={reviewDist} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} />
                <XAxis dataKey="rating" tick={{ fontSize: 12, fill: "#94a3b8" }} axisLine={{ stroke: "#e2e8f0" }} tickLine={false} />
                <YAxis tick={{ fontSize: 12, fill: "#94a3b8" }} axisLine={false} tickLine={false} allowDecimals={false} />
                <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid #e2e8f0", fontSize: 13 }} />
        ...
        <ChartCard title="Destination Popularity (by review count)" empty={!hasDestinationData} height="h-96">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={destinationPopularity} layout="vertical" margin={{ top: 10, right: 20, left: 20, bottom: 0 }}>
